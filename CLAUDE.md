...
- **Fonts** are wired in `layout.tsx` via `next/font`. A digits-only IBM Plex Sans face (`unicode-range: U+0030-0039`) leads every font stack so **all numerals render in IBM Plex automatically** while text falls through to Instrument Sans — no per-element markup. Don't reorder the stacks. Body carries `-0.02em` tracking per brand spec; `font-serif` (Instrument Serif) resets to normal tracking.
