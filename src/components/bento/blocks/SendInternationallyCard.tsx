  ...
  return (
    <div className="flex items-center justify-between rounded-xl bg-surface-1 px-4 py-6">
      <button type="button" className="flex items-center gap-2">
        ...
        <span className="font-display text-body-sm font-medium text-content">{currency}</span>
        <Icon name="chevronDown" size={16} className="text-content-secondary" />
      </button>
      <RollingNumber value={amount} className="font-sans text-h4 text-content" />
    </div>
  );
}
  ...
  return (
    <Card layoutId="send-international" className="flex flex-col gap-4">
      <div className="flex flex-col gap-1">
        <h3 className="font-display text-body-lg font-semibold text-content">{data.title}</h3>
        <p className="text-body-sm text-content-secondary">{data.subtitle}</p>
      </div>

      <div className="relative flex flex-col gap-2">
        <AmountBox currency={data.send.currency} amount={data.send.amount} />
        <span className="absolute left-1/2 top-1/2 z-10 -translate-x-1/2 -translate-y-1/2 rounded-full bg-success-muted px-3 py-1.5 font-display text-body-sm font-semibold text-success-on-muted">
          {data.rateBadge}
        </span>
